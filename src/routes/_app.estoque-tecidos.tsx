...
async function fetchLotesTecidos(): Promise<LoteAgg[]> {
...
function EstoqueTecidosPage() {
  const { data: items = [], isLoading } = useQuery({ queryKey: ["estoque-tecidos"], queryFn: fetchTecidos });
  const { data: lotes = [] } = useQuery({ queryKey: ["estoque-tecidos-lotes"], queryFn: fetchLotesTecidos });
